...
| **Notifications Read API**              | ❌ Not yet     | `GET /notifications` endpoint in marinade-notifications                             |
| **CLI `show-notifications`**            | ❌ Not yet     | Depends on Notifications Read API                                                   |
| **NotificationFormatter (rich format)** | ❌ Not yet     | v1 sends plain text to Telegram; dataPoints available for future use                |
...
3. ~~**Telegram deep link flow details**~~ — ✅ RESOLVED. Already implemented in marinade-notifications. The subscribe response for Telegram returns a deep link URL.
...
5. ~~**bonds-event-v1 codegen**~~ — ✅ RESOLVED. JSON Schema codegen pipeline implemented. `BondsEventV1` types are generated from `bonds-event-v1.json` schema and imported in all three places (bonds-eventing, bonds-notification, marinade-notifications notification-service).

6. **Notifications Read API** — `GET /notifications` endpoint in marinade-notifications is not implemented. Needed for CLI `show-notifications` and PSR dashboard pull. This blocks the CLI announcements migration path.

7. **`@marinade.finance/bonds-notification` publishing** — Currently consumed via local file link (`link:../../validator-bonds/packages/bonds-notification`). Needs to be published to npm for production deployment of marinade-notifications.

8. **marinade-notifications SPEC.md & ARCHITECTURE.md** — After implementing the subscription endpoint changes and bonds-event-v1 consumer, revisit and update `SPEC.md` and `ARCHITECTURE.md` in the marinade-notifications repo to reflect the new subscription auth model (Solana off-chain message signing), the bonds notification plugin, and the bonds-event-v1 topic.
