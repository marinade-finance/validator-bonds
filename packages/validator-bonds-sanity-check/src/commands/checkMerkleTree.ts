...
import {
  parseSettlementMerkleTree,
  parseUnifiedMerkleTree,
} from '../dtoMerkleTree'
...
function extractMetrics(
  dto: SettlementMerkleTreesDto | UnifiedMerkleTreesDto,
): MerkleTreeMetrics {
...
async function manageCheckMerkleTree({
  ...
  // Try to parse as unified merkle tree first, fallback to standard format
  let merkleTreesDto: SettlementMerkleTreesDto | UnifiedMerkleTreesDto
  let isUnifiedFormat = false
  try {
    merkleTreesDto = await parseUnifiedMerkleTree(merkleTreesData, merkleTrees)
    isUnifiedFormat = true
    const unifiedDto = merkleTreesDto as UnifiedMerkleTreesDto
    logger.info(
      `Loaded unified merkle trees: epoch ${merkleTreesDto.epoch}, sources: ${unifiedDto.sources?.join(', ') || 'N/A'}`,
    )
  } catch {
    merkleTreesDto = await parseSettlementMerkleTree(
      merkleTreesData,
      merkleTrees,
    )
    logger.info(
      `Loaded standard merkle trees: epoch ${merkleTreesDto.epoch}, count: ${merkleTreesDto.merkle_trees.length}`,
    )
  }
  ...
  if (settlementSources && settlementSources.length > 0) {
    ...
    // Total amounts should match (claims may be merged, so count can differ)
  ...
  if (pastMerkleTrees && pastMerkleTrees.length > 0) {
    ...
    const historicalDtos: (SettlementMerkleTreesDto | UnifiedMerkleTreesDto)[] =
      []
    for (const { data, sourcePath: pastPath } of pastDataWithPaths) {
      ...
      try {
        ...
        const dto = await parseSettlementMerkleTree(data, pastPath)
        historicalDtos.push(dto)
      }
    }
  ...
  if (
    isUnifiedFormat &&
    'sources' in merkleTreesDto &&
    merkleTreesDto.sources
  ) {
