...
describe('parseSettlements typed-split discrimination', () => {
  ...
  it('rejects legacy pre-refactor format with a clear error', async () => {
    ...
    await expect(parseSettlements(legacy, '/tmp/legacy.json')).rejects.toThrow(
      /legacy pre-refactor format/,
    )
  })
