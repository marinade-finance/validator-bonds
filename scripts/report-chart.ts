...
function loadMinSolRevenue(): number | null {
  try {
    const cfg = parseYaml(
      fs.readFileSync('settlement-config.yaml', 'utf8'),
    ) as {
      fee_config?: { min_sol_revenue?: number }
    }
    return cfg.fee_config?.min_sol_revenue ?? null
  } catch {
    return null
  }
}
...
async function main() {
  ...
  type PeriodData = { feeAdj: number; feeMax: number }
  const periodMap = new Map<string, PeriodData>()
  for (const r of rows) {
    const k = periodOf(r.time)
    const e = periodMap.get(k) ?? { feeAdj: 0, feeMax: 0 }
    e.feeAdj += r.feeAdj
    e.feeMax += r.feeMax
    periodMap.set(k, e)
  }
  ...
  const S_ACTUAL_BPS = 'actual_fee_bps'
  const feeBpsTidy = rows.flatMap(r => [
    { epoch: r.epoch, series: S_ADJ_MAX, bps: r.adjMaxFee / 100 },
    { epoch: r.epoch, series: S_ADJ_MIN, bps: r.adjMinFee / 100 },
    { epoch: r.epoch, series: S_ACTUAL_BPS, bps: r.actualFeeBps / 100 },
  ])

  const bpsDomainMax = Math.ceil((Math.max(maxFeeBps, maxAdjFee) * 1.1) / 100)
  const bpsYScale = { domain: [0, bpsDomainMax] } as const
  ...
  const spec: TopLevelSpec = {
    ...
    vconcat: [
      ...
      // ── Panel 2: Fee Extraction by period ───────────────────────────────────
      {
        width: 960,
        height: 210,
        title: {
      ...
      // ── Panel 3: Validators (left) + Optimized Fee % (right) ─────────────
      {
        spacing: 24,
            ...
            layer: [
              {
                ...
                encoding: {
                  ...
                  color: {
                    field: 'series',
                    scale: {
                      domain: [S_ADJ_MAX, S_ADJ_MIN, S_ACTUAL_BPS],
                      range: [C_ACTUAL, C_MINFEE, C_COST],
                    },
                    legend: {
                      title: null,
                      orient: 'bottom',
