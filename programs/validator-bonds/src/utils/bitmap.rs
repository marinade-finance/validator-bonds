...
impl BitmapProjection {
    ...
    pub fn debug_string(&self, data: &[u8]) -> String {
        ...
        let mut formatted_data =
                ...
                data[..last_byte_index + if last_bit_index == 0 { 0 } else { 1 }]
                .iter()
                .map(|b| format!("{:08b}", b))
                .collect::<Vec<String>>();
        if last_bit_index != 0 {
...
mod tests {
    ...
    fn test_bitmap_projection() {
        ...
        let mut data = vec![0_u8; BitmapProjection::bitmap_size_in_bytes(max_records)];
        let mut bitmap = BitmapProjection(max_records);
        BitmapProjection::check_size(max_records, &data).unwrap();

        assert_eq!(bitmap.number_of_bits(&data).unwrap(), 0);

        let last_index = max_records - 1;
        for i in 0..last_index {
            assert!(!bitmap.is_set(i, &data).unwrap());
            let (byte_index, bit_index) = BitmapProjection::bitmap_byte_index_and_bit_index(i);
            assert_eq!(data[byte_index], set_bits(bit_index));

            assert!(bitmap.try_to_set(i, &mut data).unwrap());
            assert!(bitmap.is_set(i, &data).unwrap());
            assert!(!bitmap.try_to_set(i, &mut data).unwrap());
            bitmap.set(i, &mut data).unwrap();
            assert!(bitmap.is_set(i, &data).unwrap());
            assert_eq!(bitmap.number_of_bits(&data).unwrap(), i + 1);

            assert_eq!(data[byte_index], set_bits(bit_index + 1));
            ...
            let debug_string_chunks = debug_string
                ...
                .collect::<Vec<String>>()
                .join(",");
            assert_eq!(bitmap.debug_string(&data), debug_string_chunks);
        }

        assert!(!bitmap.is_set(last_index, &data).unwrap());
        bitmap.set(last_index, &mut data).unwrap();
        assert!(bitmap.is_set(last_index, &data).unwrap());
        assert_eq!(bitmap.number_of_bits(&data).unwrap(), max_records);
    }
