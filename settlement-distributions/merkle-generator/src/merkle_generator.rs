...
/// Merkle tree metadata for a single validator
...
/// Collection of unified merkle trees from multiple settlement sources
#[derive(Default, Clone, Deserialize, Serialize)]
pub struct UnifiedMerkleTreeCollection {
    pub epoch: u64,
    pub slot: u64,
    /// The validator bonds config pubkey used to derive bond accounts
    #[serde(with = "pubkey_string_conversion")]
...
/// Generates unified merkle trees from multiple settlement sources
/// Each validator gets a single merkle tree with merged claims from all sources
pub fn generate_unified_merkle_trees(
    sources: Vec<SettlementSource>,
    config: &GeneratorConfig,
) -> anyhow::Result<UnifiedMerkleTreeCollection> {
    ...
    for (vote_account, settlements) in validator_settlements {
        ...
        let settlement_account = merkle_root
            .map(|root| find_settlement_address(&bond_account, &root.to_bytes(), epoch).0);
    ...
    Ok(UnifiedMerkleTreeCollection {
        epoch,
        slot,
...
mod tests {
    ...
    #[test]
    fn test_generate_unified_merkle_trees() {
        ...
        let result = generate_unified_merkle_trees(vec![source1, source2], &config).unwrap();
