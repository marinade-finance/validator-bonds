...
pub fn load_rewards_from_directory(
    ...
    log::info!("Loading stakers' inflation rewards...");
    ...
    log::info!("Loading Jito priority fee rewards...");
    ...
    log::info!("Loading validator inflation rewards...");
    ...
    log::info!("Loading validator MEV rewards...");
    ...
    log::info!("All reward files match epoch {}", epoch);

    log::info!("Aggregating rewards by vote account...");
...
fn aggregate_rewards(
    ...
    for reward in inflation_rewards {
        if let Some(vote_account) =
            find_vote_account_for_stake(&reward.stake_account, stake_meta_index)
        {
    ...
    log::info!(
        "Aggregated rewards (total: {}, total stakers: {}) for {} vote accounts",
        rewards_map.values().map(|r| r.total_amount).sum::<u64>(),
        rewards_map
            .values()
...
/// Find the vote account associated with a stake account using the stake meta index
fn find_vote_account_for_stake(
    ...
    stake_meta_index
        ...
        .stake_metas
        .iter()
        .find(|meta| &meta.pubkey == stake_account)
