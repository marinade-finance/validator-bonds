...
- ❌ **Notifications read API** — `GET /v1/notifications` not yet implemented
...
│  │  POST /v1/subscriptions │ │  GET /v1/notifications            │        │
│  │  DELETE /v1/subscriptions│ │  - filter: type, user_id,        │        │
│  │  GET /v1/subscriptions│   │    priority, inner_type, recency  │        │
│  │  - Solana sig verify │   │                                   │        │
│  │  ✅ IMPLEMENTED      │   │                                   │        │
│  └─────────────────────┘   └──────────────────────────────────┘        │
│                                                                         │
...
- `sam_eligible_change`: `sha256(vote_account + "sam_eligible" + currentEligible + time_bucket)` — dedup by eligibility state
...
CREATE TABLE notification_dedup (
    ...
    notification_type TEXT NOT NULL,     -- 'sam_auction', future types
...
CREATE TABLE subscriptions (
    ...
    notification_type TEXT NOT NULL,  -- 'sam_auction', future: 'staking-rewards'
...
GET /v1/subscriptions?pubkey={pubkey}&notification_type={type}
...
```
GET /v1/notifications?user_id={vote_account}&notification_type=sam_auction&priority=critical&inner_type=bond_underfunded_change&limit=50&offset=0
...
12. **marinade-notifications: notifications read API** — ✅ `GET /v1/notifications` endpoint with Solana auth, filters (type, priority, inner_type, limit/offset), broadcast inclusion.
...
20. **PSR dashboard integration** — ✅ Notification column added to SAM table. Shows `!!`/`!`/`i` indicators per validator with color (red/yellow/blue) and tooltip with notification details. Fetches from `GET /v1/notifications` (public, no auth).
...
**Phase 8: Replace CLI announcements with notifications API** — 🔧 IN PROGRESS

Replace the old `validator-bonds-api/v1/announcements` banner system with a new `GET /v1/notifications/broadcast` endpoint in marinade-notifications. Unifies the two separate notification/announcement systems in the CLI.

27. ✅ **`GET /v1/notifications/broadcast` endpoint** (marinade-notifications) — New endpoint returning only broadcast notifications (`user_id = BROADCAST_USER_ID`). No `user_id` param needed. Same relevance/deactivation filtering as other endpoints. Extract `BROADCAST_USER_ID` magic constant into shared `notifications.constants.ts`.
28. ✅ **`scope` field on notification responses** (marinade-notifications) — Computed `scope: 'broadcast' | 'individual'` field added to all notification endpoint responses. Derived from `user_id === BROADCAST_USER_ID` at query time, no new DB column.
29. ✅ **`ts-subscription-client`: add `listBroadcastNotifications()` method** — New method calling `GET /v1/notifications/broadcast`. Add `scope` field to `Notification` type. Add `NOTIFICATION_TYPE_INSTITUTIONAL_SELECT` constant.
30. ✅ **Global `--notifications-api-url` option** — Replace `--announcements-api-url` with `--notifications-api-url` as a global hidden option (env `NOTIFICATIONS_API_URL`, default `https://marinade-notifications.marinade.finance`). Store in `ValidatorBondsCliContext`. Remove per-command `--notifications-api-url` from subscribe/unsubscribe/subscriptions/show-notifications commands.
31. ✅ **Rewrite `announcements.ts` → `notifications.ts`** — Replace raw `fetch()` to old API with `createSubscriptionClient` + `listBroadcastNotifications({ notification_type })`. Keep non-blocking `preAction`/`postAction` pattern with `Promise.race` timeout. Remove `CliType` enum and class-validator DTOs.
32. ✅ **Update `banner.ts`** — Render `Notification[]` from ts-subscription-client instead of old `CliAnnouncementsResponseDto`. Use `notification.title` + `notification.message` for banner title/text. Group banners with configurable `linesAround`.
33. ✅ **Update CLI launch configs** — SAM CLI: `notificationType: 'sam_auction'`. Institutional CLI: `notificationType: 'institutional_select'`.
34. ✅ **`title` column in `notifications_outbox`** — Nullable `TEXT` column. Consumer passes `content.title` (from `buildContent()`) into `writeOutbox()`. Returned in API response. CLI `getBanner()` uses it. Old rows with `title = NULL` render without title (backward compat).
35. ✅ **`show-notifications` without address** — When no bond/vote address is provided, shows broadcast notifications instead of requiring one.
36. ❌ **Deploy marinade-notifications + publish `ts-subscription-client`** — Server must deploy first, then client package bump, then CLI update.
37. ❌ **Manual `ALTER TABLE notifications_outbox ADD COLUMN IF NOT EXISTS title TEXT`** — needed for existing DB.
