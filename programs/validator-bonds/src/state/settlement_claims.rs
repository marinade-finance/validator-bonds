...
const SETTLEMENT_CLAIMS_ANCHOR_HEADER_SIZE: usize =
    crate::constants::SETTLEMENT_CLAIMS_ANCHOR_HEADER_SIZE as usize;
...
pub fn account_size(max_records: u64) -> usize {
    SETTLEMENT_CLAIMS_ANCHOR_HEADER_SIZE + BitmapProjection::bitmap_size_in_bytes(max_records)
}
...
impl<'info, 'a> SettlementClaimsWrapped<'info, 'a> {
    pub fn new(account: &'a Account<'info, SettlementClaims>) -> Result<Self> {
        ...
        if BitmapProjection::check_size(
            account.max_records,
            &account_info.data.borrow_mut()[SETTLEMENT_CLAIMS_ANCHOR_HEADER_SIZE..],
        )
        .is_err()
        {
            ...
            return Err(
                error!(crate::ErrorCode::SettlementClaimsNotInitialized).with_values((
                    ...
                    format!(
                        "'{}/{}' : '{}'",
                        account.max_records,
                        SETTLEMENT_CLAIMS_ANCHOR_HEADER_SIZE + bitmap_byte_size,
    ...
    pub fn try_to_set(&mut self, index: u64) -> Result<bool> {
        self.bitmap_projection.try_to_set(
            index,
            &mut self.account_info.data.borrow_mut()[SETTLEMENT_CLAIMS_ANCHOR_HEADER_SIZE..],
        )
    }
}
        ...
        f.debug_struct("SettlementClaims")
            ...
            .field(
                ...
                &self.bitmap_projection.debug_string(
                    &self.account_info.data.borrow()[SETTLEMENT_CLAIMS_ANCHOR_HEADER_SIZE..],
                ),
            )
            .finish()
    }
