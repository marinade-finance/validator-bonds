...
if (!epochArg || fees.length === 0) {
  process.stderr.write("usage: bun scripts/fee-sensitivity.ts [-r] [-v] <epoch|start-end> <fees_bps>... [--data-dir DIR]\n");
  process.exit(2);
}
...
const ssrFeed = await ssrRes.json() as { epochs: { epoch: number; pmpe: number; time: number }[] };

const INPUTS = [
  "stakes.json", "sam-scores.json", "validators.json", "evaluation.json",
...
const cli = ["cargo", "run", "-q", ...(values.r ? ["--release"] : []), "--bin", "bid-distribution-cli", "--"];
const apy = (p: number, n: number) => ((Math.pow(1 + p / 1000, n) - 1) * 100).toFixed(2) + "%";
...
for (let epoch = epochStart; epoch <= epochEnd; epoch++) {
  ...
  if (!INPUTS.every(f => existsSync(join(inp, f)))) {
    ...
    Bun.spawnSync(["./scripts/regression-test-settlements.sh",
      "--start-epoch", String(epoch), "--end-epoch", String(epoch), "--data-dir", dataDir],
      { stderr: "inherit" });
    if (!INPUTS.every(f => existsSync(join(inp, f)))) {
  ...
  if (!eData) { process.stderr.write(`  # epoch ${epoch} not in SSR feed, skipping\n`); continue; }
  ...
  for (const fee of fees) {
    ...
    const proc = Bun.spawnSync([...cli,
      "--settlement-config", cfg,
      "--stake-meta-collection", `${inp}/stakes.json`,
      "--sam-meta-collection", `${inp}/sam-scores.json`,
      "--rewards-dir", `${inp}/rewards`,
      "--validator-meta-collection", `${inp}/validators.json`,
      "--revenue-expectation-collection", `${inp}/evaluation.json`,
    ...
    const ncap = bids.filter(d => {
      const t = parseFloat(d.total_marinade_stakers_rewards);
      return t > 0 && (d.marinade_fee_claim + d.dao_fee_claim) < t * fee / 10000 * 0.9999;
    }).length;
