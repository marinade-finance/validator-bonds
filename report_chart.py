...
# semantic palette — red reserved for cost/shortfall only
C_ACTUAL   = "steelblue"
C_REF      = "slategray"
C_CAP      = "seagreen"
...
def main():
    ...
    style(ax2, "Fee Revenue (SOL)  —  adjusted vs max-fee", "SOL")
    w = 0.35
    ax2.bar(xs - w/2, fee_adj, w, color=C_ACTUAL, alpha=0.85, label="Adjusted")
    ax2.bar(xs + w/2, fee_max, w, color=C_REF,    alpha=0.70, label="Max-fee")
    ax2.legend(fontsize=9)
    ax2.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"{v:.0f}"))

    # ── 3. Validators at cap / at min fee ─────────────────────────────────────
    ax3 = fig.add_subplot(gs[1, 1])
    style(ax3, "Validators at Fee Cap / at Min Fee (%)", "%")
    ax3.fill_between(xs, vcap, alpha=0.10, color=C_CAP)
    ax3.plot(xs, vcap, lw=2, marker="o", markersize=4, color=C_CAP,     label="At cap (≥ max_fee)")
    ...
    for i, v in enumerate(vcap):
        ax3.annotate(f"{v:.0f}%", (xs[i], v),
                     textcoords="offset points", xytext=(0, 5),
                     fontsize=7.5, ha="center", color=C_CAP)
    for i, v in enumerate(vmin):
        ax3.annotate(f"{v:.0f}%", (xs[i], v),
                     textcoords="offset points", xytext=(0, -11),
                     fontsize=7.5, ha="center", color=C_MINFEE)
    ax3.set_ylim(0, 115)
    ax3.legend(fontsize=9)
    ax3.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"{v:.0f}%"))

    # ── 4. Shortfall per epoch (full width) ───────────────────────────────────
    ax4 = fig.add_subplot(gs[2, :])
    style(ax4, f"Cost to Marinade — Fee Shortfall per Epoch (SOL)  [total: {sum(shortfall):.0f} SOL]", "SOL")
    ax4.bar(xs, shortfall, color=C_COST, alpha=0.80, label="Shortfall (max − adjusted)")
