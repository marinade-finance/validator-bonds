use crate::generators::fee_optimizer::{calculate_bid_settlement_totals, generate_bid_settlements};
...
fn test_activating_fee_fraction_falls_back_when_distributor_takes_all_active() {
    ...
    let staker_total = bidding
        .claims
        .iter()
        .filter(|c| matches!(c.detail, ClaimDetail::StakerPayout { .. }))
        .map(|c| c.claim_amount)
        .sum::<u64>();
    assert_eq!(
        staker_total, 0,
...
#[test]
fn test_calculate_bid_settlement_totals_priority_fallback_excludes_activating_stake() {
    // Activating-only validator (no Bidding settlement → PriorityFee fallback path):
    // its activating_bid_claim feeds the global yield numerator while only its ACTIVE
    // stake feeds the denominator. Pinned intentionally: the yield is normalized by
    // active stake everywhere (Bidding branch + split_distributor_fee both fold the
    // activating bid into the numerator and exclude activating stake), so counting
    // activating stake here only would skew activating-only validators vs mixed ones.
    let vote = test_vote_account(7);
    let settlement = Settlement {
        ...
        details: Some(SettlementDetails::PriorityFee(
            PriorityFeeSettlementDetails {
                total_marinade_active_stake: 0,
                total_marinade_activating_stake: 5_000_000,
                activating_stake_pmpe: "100".to_string(),
                activating_bid_claim: "500".to_string(),
                activating_stakers_pool: 0,
                marinade_fee_claim: 40,
                dao_fee_claim: 10,
            },
        )),
    };
    let totals = calculate_bid_settlement_totals(&[settlement]);
    assert!(
        totals.stake.is_zero(),
        "activating stake must NOT enter the denominator, got {}",
        totals.stake
    );
    assert_eq!(
        totals.rewards,
        Decimal::from(500),
        "activating_bid_claim must enter the numerator"
    );
    assert_eq!(totals.fees, Decimal::from(50));
}
