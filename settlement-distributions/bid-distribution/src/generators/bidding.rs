...
fn split_distributor_fee(
    ...
    info!(
        ...
        vote_account, effective_fee, fee_percentages.max_fee, fee_percentages.min_fee, ssr_pmpe,
    );
...
pub(crate) fn generate_bid_settlements_worker(
    ...
    for (validator, staker_stake_groups) in validator_groups {
        ...
        let fee = split_distributor_fee(
            ...
            &fee_percentages,
            ssr_pmpe,
            fee_config.min_yield_premium_over_ssr_pmpe,
            &validator.vote_account,
        )?;
