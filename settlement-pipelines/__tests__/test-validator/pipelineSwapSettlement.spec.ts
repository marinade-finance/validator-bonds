...
// wallet and replaced by a freshly created stake the instruction mints from the
// marinade wallet's SOL, re-delegated and deactivated THIS epoch — so the
...
describe('Pipeline fund-settlement swap', () => {
  ...
  it('funds a bond settlement and swaps it claimable in the same epoch', async () => {
    ...
    // marinade wallet: funds (and signs as caller for) the freshly created
    // replacement stake; it ends up owning the settlement's original bond stake
    ...
    // before the swap the marinade wallet holds no stake accounts
    const marinadeStakesBefore = await findStakeAccounts({
      connection: provider.connection,
      staker: marinadeWallet.publicKey,
    })
    expect(marinadeStakesBefore.length).toEqual(0)
    ...
    // the swap handed the settlement's original bond stake to the marinade wallet
    const marinadeStakesAfter = await findStakeAccounts({
      connection: provider.connection,
      staker: marinadeWallet.publicKey,
      currentEpoch,
    })
    expect(marinadeStakesAfter.length).toBeGreaterThanOrEqual(1)

    // the settlement-owned stake is now the freshly minted replacement, delegated
    // to the validator THIS epoch and deactivated the same epoch -> claimable now
