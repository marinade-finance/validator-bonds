...
// number of bytes in 10MB * 8; 56 bytes is size for account header
// as we know there is 56 bytes for account header it fits 8 times more items
const maxMerkleNodesAccountSize =
  tenKilobytes * 1024 * 8 - 8 * SETTLEMENT_CLAIMS_ANCHOR_HEADER_SIZE
...
describe('Validator Bonds init settlement', () => {
  ...
  it('cannot init settlement with too many records', async () => {
    ...
    const { instruction: ixFail, settlementAccount: failureAccount } =
      await initSettlementInstruction({
        ...
        operatorAuthority,
        merkleRoot,
        maxMerkleNodes: maxMerkleNodesAccountSize + 1,
        maxTotalClaim: 111,
        voteAccount,
        epoch: await currentEpoch(provider),
        configAccount,
    ...
    try {
      ...
      verifyError(e, Errors, 6067, 'exceed maximum to fit Solana')
    }
  ...
  it('init settlement not-fully initialized', async () => {
    ...
    const {
      ...
      operatorAuthority,
      merkleRoot,
      maxMerkleNodes: maxMerkleNodesAccountSize,
      maxTotalClaim: 42,
      voteAccount,
      epoch: await currentEpoch(provider),
      configAccount,
