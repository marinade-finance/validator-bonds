...
fn ssi_stake_meta_index() -> (StakeMetaCollection, Pubkey) {
    let vote_account = test_vote_account(10);
    // Staker has stake_authority=MARINADE (passes filter) but withdraw=unique authority.
    // No fee deposit account in the index — fee claims land at MARINADE/MARINADE with empty
    // stake_accounts, keeping them separate from the staker claim.
...
fn test_ssi_fee_cap_active_reduces_fee() {
    ...
    // 1000 SOL * 20/1000 = 20 SOL stakers_rewards → marinade_fee = 5 SOL
    let settlements = run_ssi_test(15.0, ssi_fee_config(3000, 0, 0.0));
    let marinade_fee = sum_claims_for_authority(&settlements, &TEST_PUBKEY_MARINADE, &TEST_PUBKEY_MARINADE);
    assert_eq!(
        marinade_fee,
...
fn test_ssi_fee_cap_not_binding_keeps_configured_fee() {
    ...
    // marinade_fee = 20 SOL * 0.30 = 6 SOL
...
fn test_ssi_fee_cap_zero_when_ssi_exceeds_yield() {
    // staker_yield_pmpe=20, ssi=25 → fee_cap=0, min_fee=0 → effective=0 → no fee
    let settlements = run_ssi_test(25.0, ssi_fee_config(3000, 0, 0.0));
    let marinade_fee = sum_claims_for_authority(&settlements, &TEST_PUBKEY_MARINADE, &TEST_PUBKEY_MARINADE);
    // staker gets full 20 SOL static_bid claim; Marinade fee = 0
    let staker_claim = sum_claims_for_authority(&settlements, &TEST_PUBKEY_MARINADE, &TEST_PUBKEY_MARINADE);
    assert_eq!(staker_claim, 0, "fee must be zero when SSI exceeds staker yield");
    let _ = settlements; // stakers receive full static bid
}

#[test]
fn test_ssi_min_fee_overrides_fee_cap() {
    // staker_yield_pmpe=20, ssi=19 → fee_cap=0.05, min_fee=1000bps=0.10 → effective=0.10
    // marinade_fee = 20 SOL * 0.10 = 2 SOL
