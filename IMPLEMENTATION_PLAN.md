...
| **Notifications Read API**                               | ✅ Implemented | `GET /v1/notifications` public endpoint (no auth) in marinade-notifications                                 |
...
| **Part L: Replace announcements with notifications API** | 🔧 In progress | Announcements → notifications rewrite done; `title` column added to outbox; pending deploy + client publish |
...
- `inner_type` enum expanded from the original schema to include delta types: `first_seen`, `bond_removed`, `auction_entered`, `auction_exited`, `cap_changed`, `bond_underfunded_change`, `bond_balance_change`, `sam_eligible_change` (plus original `announcement`, `version_bump`)
...
5. POST to `{notifications-api-url}/v1/subscriptions`:
   ```json
   {
...
Same signing flow as subscribe, but with `"Unsubscribe bonds <type> <timestamp>"` message text. Calls `DELETE /v1/subscriptions`. `--address` is optional — when omitted, unsubscribes from all subscriptions of the given type; when provided, unsubscribes only the specific address.
...
3. GET `{notifications-api-url}/v1/subscriptions?pubkey={pubkey}&notification_type=sam_auction` with `x-solana-signature` and `x-solana-message` headers
...
A future `show-notifications` command to fetch delivered notifications from `GET /v1/notifications` may be added when the notifications read API exists. This would support `--priority`, `--limit`, `--inner-type` filters.
...
Step 11: Implement subscriptions (show) command + tests        ✅ DONE (cf3d4fe)
  ...
  ├── Auth via x-solana-signature + x-solana-message headers on GET /v1/subscriptions
...
6. ~~**Notifications Read API**~~ — ✅ RESOLVED. `GET /v1/notifications` endpoint + CLI `show-notifications` implemented.
...
- **~~Unsubscribe HTTP method~~** — Resolved. `DELETE /v1/subscriptions` (confirmed from marinade-notifications subscription controller implementation).
...
**Fix:** CLI now sends `additional_data: { config_address, vote_account, bond_pubkey }` for read operations (`packages/validator-bonds-cli-core/src/commands/manage/subscriptions.ts`). Server-side `GET /v1/subscriptions` in marinade-notifications parses `additional_data` from query parameter and passes it to the verifier.
...
- Notifications Read API `GET /v1/notifications` (deferred)
...
| `sam_eligible_change`     | `sam_eligible`   | `currentEligible` (string)              | — (no renotify)   |
...
CREATE TABLE notification_dedup (
    ...
    notification_type TEXT NOT NULL,          -- 'sam_auction', 'staking-rewards', etc.
...
- **`deactivated_at` for soft-delete** — admin can deactivate any notification (especially announcements) via `PATCH /v1/notifications/{id}/deactivate` which sets `deactivated_at = NOW()`. The row stays in DB for audit trail. Read API and dashboard filter it out. No row deletion needed.
...
private async resolveTargets(
  ...
  const subscribed = await this.subscriptions.getSubscribedChannels(userId, 'sam_auction')
...
private async resolveTarget(
  ...
  if (sub.channel === 'telegram') {
    ...
    const chatId = await this.subscriptions.getTelegramChatId(
      userId, 'sam_auction', sub.channelAddress,
    )
    if (!chatId) {
...
private async writeOutbox(
  ...
  await this.rds.pool.query(sql.unsafe`
    ...
    VALUES
      (${'sam_auction'}, ${event.inner_type}, ${outboxUserId}, ${evaluation.priority},
...
Step 9: Notifications read API + deactivation (marinade-notifications repo)
  ...
  ├── GET /v1/notifications: query notifications_outbox with filters + deactivated_at IS NULL
  ├── PATCH /v1/notifications/:id/deactivate: soft-delete (JWT auth, same AuthGuard as ingress)
  ├── Solana signature auth for GET (read own notifications)
  └── Tests: read with filters, auth verification, deactivation, empty results
  → verify: GET /v1/notifications returns correct data; deactivation hides entries
...
Validator reads notifications:
  ...
  ├── CLI: GET /v1/notifications → reads from notifications_outbox
  └── PSR Dashboard: GET /v1/notifications → reads from notifications_outbox
```
...
| File                                  | Purpose                                                            |
| ------------------------------------- | ------------------------------------------------------------------ |
| `subscriptions.controller.ts`         | POST/DELETE/GET `/v1/subscriptions` endpoints + Telegram webhook   |
...
```typescript
subscribeMessage('sam_auction', 'telegram', timestampSeconds)
// → 'Subscribe sam_auction telegram 1710000000'

unsubscribeMessage('sam_auction', 'email', timestampSeconds)
...
The PSR dashboard (`/home/chalda/marinade/psr-dashboard`) currently has a hardcoded banner system (`src/services/banner.tsx`). It should poll the `GET /v1/notifications` API to show bond-specific notifications per validator row.

### I.2 Auth — RESOLVED

`GET /v1/notifications` is a **public endpoint** (no auth). Notification data is derived from on-chain state and is not sensitive. Only subscription management requires Solana signature auth. The PSR dashboard can query directly for any vote_account.
...
export async function fetchNotifications(
  ...
  const url = new URL('/v1/notifications', notificationsApiUrl)
...
> **bonds-event-v1** — Schema: `message-types/schemas/bonds-event-v1.json`. 10 inner_types (delta-based: first_seen, bond_removed, auction_entered, auction_exited, cap_changed, bond_underfunded_change, bond_balance_change, sam_eligible_change, announcement, version_bump). Generated package: `bonds-event-v1` (types + Ajv validator). Consumer uses external brain library for evaluation.
...
## Part L: Replace CLI Announcements with Notifications API

### Problem

The CLI has two separate systems for showing server-side messages:

1. **Old announcements** — `announcements.ts` fetches from `validator-bonds-api/v1/announcements` (Rust API backed by `cli_announcements` PostgreSQL table), rendered as banners via `preAction`/`postAction` hooks in `mainCommand.ts`
2. **show-notifications** — fetches from `marinade-notifications/v1/notifications` via `ts-subscription-client`

The `--notifications-api-url` option only exists on subscription-related commands (subscribe, unsubscribe, subscriptions, show-notifications). Running e.g. `show-bond --notifications-api-url http://localhost:3000` fails with "unknown option".

### Goal

- Add `GET /v1/notifications/broadcast` endpoint to marinade-notifications — returns only broadcast notifications (no `user_id` needed)
- Add computed `scope: 'broadcast' | 'individual'` field to all notification responses — derived from `user_id` vs `BROADCAST_USER_ID` constant, no new DB column
- Make `--notifications-api-url` a single global option available on all commands
- Replace old announcements with `GET /v1/notifications/broadcast` from marinade-notifications
- Add `institutional_select` notification type for the institutional CLI variant
- Remove per-command `--notifications-api-url` options (use context instead)
- Remove old `CliType` enum and class-validator DTOs

### Notifications API Endpoints (marinade-notifications)

Existing endpoints in `notification-service/notifications/notifications.controller.ts`:

- `GET /v1/notifications` — requires `user_id` query param, returns user-specific + broadcast (`MarinadeNotifications1111111111111111111111`) notifications
- `GET /v1/notifications/all` — no `user_id` required, returns all notifications (broadcast + all users mixed together)

**New endpoint:**

- `GET /v1/notifications/broadcast` — no `user_id` required, returns only broadcast notifications (`user_id = BROADCAST_USER_ID`). Supports same filters: `notification_type`, `priority`, `inner_type`, `limit`, `offset`. Same `relevance_until > now()` and `deactivated_at IS NULL` filtering.

**Broadcast mechanism:** When `force: true` in routing config (`bonds-routing.yml`), the consumer writes to `notifications_outbox` with `user_id = 'MarinadeNotifications1111111111111111111111'`. Currently this constant is defined locally in `notifications.service.ts:8`. It should become a shared constant.

All endpoints are public (no auth). Rate limited to 60 req/min.

### Step L.1: marinade-notifications — `BROADCAST_USER_ID` constant + `scope` field + `/broadcast` endpoint

**File: `notification-service/notifications/notifications.constants.ts`** (new)

Extract `BROADCAST_USER_ID` into a shared constant file:

```typescript
export const BROADCAST_USER_ID = 'MarinadeNotifications1111111111111111111111'
export type NotificationScope = 'broadcast' | 'individual'
```

**File: `notification-service/notifications/notifications.service.ts`**

- Import `BROADCAST_USER_ID` from constants (remove local definition)
- Add `getBroadcastNotifications()` method:
  ```sql
  WHERE user_id = BROADCAST_USER_ID
    AND deactivated_at IS NULL
    AND relevance_until > now()
    AND (type/priority/innerType filters)
  ORDER BY created_at DESC
  LIMIT / OFFSET
  ```

**File: `notification-service/notifications/notifications.controller.ts`**

- Import `BROADCAST_USER_ID`, `NotificationScope` from constants
- Add computed `scope` field to `NotificationResponse` interface:
  ```typescript
  scope: NotificationScope
  ```
- In `toResponse()`, derive scope:
  ```typescript
  scope: n.user_id === BROADCAST_USER_ID ? 'broadcast' : 'individual'
  ```
- Add `@Get('broadcast')` handler (before the existing `@Get()` to avoid route conflicts):
  ```typescript
  @Get('broadcast')
  async listBroadcast(...filters): Promise<NotificationResponse[]> {
    const notifications = await this.service.getBroadcastNotifications({ ...filters })
    return notifications.map(n => this.toResponse(n))
  }
  ```
- Existing `list()` and `listAll()` also get the `scope` field via `toResponse()` — no behavior change, just enriched response

**File: `notification-service/consumers/bonds-event-v1/consumer.ts`**

- Import `BROADCAST_USER_ID` from `../notifications/notifications.constants` (remove hardcoded string in `writeOutbox()`)

### Step L.1b: `title` column in `notifications_outbox`

**Problem:** The old announcements had `title` + `text`. The new notifications only have `message`. The CLI `getBanner()` already supports a `title` parameter. The `buildContent()` function in `notifications-bonds/src/content.ts` already computes titles per `inner_type` (e.g., "Bond Underfunded", "Removed from Auction") — but the title was only used for Telegram delivery, not stored in the outbox.

**Solution:** Add a nullable `title TEXT` column to `notifications_outbox`. Populate it at write time from `content.title`. Return it in the API response.

**Migration:** `ALTER TABLE notifications_outbox ADD COLUMN IF NOT EXISTS title TEXT;` — also added to `04-bonds-event-v1.sql` for fresh installs.

**Changes:**

- `notification-service/migrations/04-bonds-event-v1.sql` — add `title TEXT` to CREATE TABLE
- `notification-service/consumers/bonds-event-v1/consumer.ts` — `writeOutbox()` accepts `title` param, includes in INSERT; caller passes `content.title`
- `notification-service/notifications/notifications.service.ts` — add `title: string | null` to `Notification` interface, add `title` to SELECT
- `notification-service/notifications/notifications.controller.ts` — add `title: string | null` to `NotificationResponse`, include in `toResponse()`
- `ts-subscription-client/types.ts` — add `title: string | null` to `Notification`
- `packages/validator-bonds-cli-core/src/banner.ts` — pass `notification.title` to `getBanner()`

**Backward compat:** Column is nullable. Old rows have `title = NULL`. `getBanner()` renders without title when undefined. `buildContent()` always returns a title (fallback: `inner_type` itself), so all new rows will have a title.

### Step L.2: `ts-subscription-client` — add `listBroadcastNotifications()` + `scope` field

**File: `ts-subscription-client/types.ts`**
...
Add `scope: NotificationScope` to existing `Notification` interface.

**File: `ts-subscription-client/client.ts`**

Add method to `SubscriptionClient`:
...
async listBroadcastNotifications(
  ...
  if (query.inner_type) params.set('inner_type', query.inner_type)
  ...
  const path = `${PATH_NOTIFICATIONS}/broadcast${params.toString() ? '?' + params.toString() : ''}`
  this.logger?.debug(`GET ${path}`)
  return this.fetch<Notification[]>(path, { method: 'GET' })
}
```

**File: `ts-subscription-client/index.ts`**
...
### Step L.3: Global `--notifications-api-url` option + context (validator-bonds repo)

**File: `packages/validator-bonds-cli-core/src/context.ts`**

Add `notificationsApiUrl: string` field to `ValidatorBondsCliContext` constructor and `setValidatorBondsCliContext()`.

**File: `packages/validator-bonds-cli-core/src/commands/mainCommand.ts`**

- Replace `--announcements-api-url` option with:
  ```typescript
  .addOption(
    new Option('--notifications-api-url <url>', 'Override notifications API URL')
      .env('NOTIFICATIONS_API_URL')
      .default('https://marinade-notifications.marinade.finance')
      .hideHelp(),
  )
  ```
- Replace `announcementsConfig?: AnnouncementsConfig` param with `notificationsConfig?: { enabled: boolean, notificationType: string }`
- In `preAction`: pass `notificationsApiUrl` to `setValidatorBondsCliContext()`, call `startFetchingNotificationBanners()` with `notificationType` and `notificationsApiUrl`
- In `postAction`: call `printNotificationBanners()` (renamed)

### Step L.4: Rewrite `announcements.ts` (validator-bonds repo)

**File: `packages/validator-bonds-cli-core/src/announcements.ts`**

Remove:

- `CliAnnouncementDto`, `CliAnnouncementsResponseDto` (class-transformer/class-validator DTOs)
- `CliType` enum, `AnnouncementsConfig` interface
- `ANNOUNCEMENTS_API_URL`, `ANNOUNCEMENTS_TIMEOUT_MS` constants
- `buildAnnouncementUrl()`, `fetchAnnouncementsInternal()` (raw fetch logic)
- `startFetchingAnnouncements()`, `getAnnouncements()`, `clearAnnouncementCache()`

Add:

- `NotificationsConfig` interface: `{ enabled: boolean, notificationType: string }`
- `NOTIFICATION_BANNER_TIMEOUT_MS = 1500`
- `startFetchingNotificationBanners(params: { notificationType: string, apiUrl: string }, logger?)` — creates `SubscriptionClient` (with `timeout_ms: NOTIFICATION_BANNER_TIMEOUT_MS`), calls `listBroadcastNotifications({ notification_type: params.notificationType, limit: 10 })`, stores promise
- `getNotificationBanners(timeoutMs?)` — races stored promise against timeout, returns `Notification[] | null`
- `clearNotificationBannerCache()` — for testing

Note: no `inner_type: 'announcement'` filter — the `/broadcast` endpoint already returns only broadcast (force: true) notifications, which is exactly the announcement subset.

### Step L.5: Update `banner.ts` (validator-bonds repo)

**File: `packages/validator-bonds-cli-core/src/banner.ts`**

- Rename `printAnnouncementBanners()` → `printNotificationBanners()`
- Change import from old `getAnnouncements` to new `getNotificationBanners`
- Iterate over `Notification[]` instead of `CliAnnouncementsResponseDto.announcements`
- Use `notification.message` as banner text
- Use `notification.title` for banner title (nullable — `getBanner()` renders without title when null, for backward compat with old rows)
- Keep `Color.Bold` style (no priority-based colors)

### Step L.6: Remove per-command `--notifications-api-url` options

**Core files (remove `.addOption()` and get URL from context via `getCliContext().notificationsApiUrl`):**

- `packages/validator-bonds-cli-core/src/commands/manage/subscribe.ts` — also remove `NOTIFICATIONS_API_URL_ENV`, `NOTIFICATIONS_API_URL_DEFAULT` exports
- `packages/validator-bonds-cli-core/src/commands/manage/unsubscribe.ts`
- `packages/validator-bonds-cli-core/src/commands/manage/subscriptions.ts`
- `packages/validator-bonds-cli-core/src/commands/manage/show-notifications.ts`

**Wrapper files (remove `notificationsApiUrl` from action handler destructuring):**

- `packages/validator-bonds-cli/src/commands/manage/show-notifications.ts`
- `packages/validator-bonds-cli/src/commands/manage/subscribe.ts`
- `packages/validator-bonds-cli/src/commands/manage/unsubscribe.ts`
- `packages/validator-bonds-cli/src/commands/manage/subscriptions.ts`

### Step L.7: Update CLI launch configs

**File: `packages/validator-bonds-cli/src/index.ts`**
...
**File: `packages/validator-bonds-cli-institutional/src/index.ts`**
...
### Step L.8: Cleanup exports

**File: `packages/validator-bonds-cli-core/src/index.ts`**

- Remove exports: `CliType`, `AnnouncementsConfig`
- Add exports: `NotificationsConfig` (if needed by external consumers)

### Verification

1. `pnpm cli show-bond <address> --notifications-api-url http://localhost:3000` — global option works
2. `pnpm cli show-notifications <address>` — still works, uses global URL from context
3. `pnpm cli subscribe <address> --type email --address test@test.com` — still works
4. Banner appears after commands when broadcast notifications exist
5. `pnpm fix` passes in all modified packages
